...
getVec <- function(dataset, prop = TRUE){
  ...
  x <- dataset$date2
  c1 <- dataset$Prc_susp_501Y_V1/100 * nn
  c23 <- c1 + dataset$Prc_susp_501Y_V2_3/100 * nn
  cABS <- c23 + dataset$Prc_susp_ABS/100 * nn
  cIND <- cABS + dataset$Prc_susp_IND/100 * nn
  list(x = x, c1 = c1, c23 = c23, cABS = cABS, cIND = cIND)
}

colV <- c(brewer.pal(3, "Set2"), gray(0.5))

plotDist <- function(x, c1, c23, cABS, cIND){
  if(max(cIND, na.rm = TRUE) == 1 | max(cIND, na.rm = TRUE) == 100 | max(c1, na.rm = TRUE) > 100){
    yl <- "Proportions variants"
  }else{
    yl <- "Number cases"
  }
  plot(x, c1, ylim = c(0, max(cIND)), type = "l", 
...
plotDist(cc$x, cc$c1, cc$c23, cc$cABS, cc$cIND)
```
